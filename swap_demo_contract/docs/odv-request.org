...
To calculate the minimum C3 tokens needed, a user must multiply the `node_fee` by the number of nodes required, then add the `aggregate_fee` and `platform_fee`. In this demo, the calculation is as follows: 1.5 * 2 + 2.5 + 1 = 6.5.
